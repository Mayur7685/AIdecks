// Admin hook — Admin ops go through backend (which holds admin key).

import { useState, useCallback } from 'react';
import { readContract, getLedgerSequence, formatXTZ, STARTUPS } from '../lib/stellar';
import { STELLAR_ADMIN, STELLAR_RPC_URL } from '../lib/networks';
...
export function useAdmin() {
    ...
    const getAdminStats = useCallback(async (): Promise<AdminStats> => {
        const [packsSoldStr, packPriceStr, totalCardsStr] = await Promise.all([
            readContract('total_packs_sold', '0u8'),
            readContract('pack_price', '0u8'),
            readContract('total_cards_minted', '0u8'),
        ]);
        ...
        for (let id = 1; id <= 19; id++) {
            try {
                const ed = await readContract('startup_editions', `${id}u8`);
    ...
    const getTournaments = useCallback(async (): Promise<TournamentData[]> => {
        ...
        const nextIdStr = await readContract('next_tournament_id', '0u8');
    ...
    const callBackend = async (path: string, body: any = {}) => {
        const res = await fetch(apiUrl(`/api/admin${path}`), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-Admin-Key': localStorage.getItem('aidecks:admin-key') || '' },
    ...
    const createTournament = useCallback(async (
        ...
        try {
            ...
            const res = await fetch(`${STELLAR_RPC_URL}/${'testnet'}/block/height/latest`);
